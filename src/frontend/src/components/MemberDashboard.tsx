  ...
  return (
    <>
      ...
      @keyframes glowRingPulse {
        0%   { box-shadow: 0 0 0 0px var(--glow-col, rgba(6,182,212,0.9)); }
          ...
          >
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {active === "activity" && <ActivityPanel handle={handle} />}
...
// ---- Activity Feed Panel ----
function ActivityPanel({ handle }: { handle: string }) {
  const ACTIVITY_ITEMS = [
    {
      id: 1,
      icon: "🔐",
      type: "auth",
      text: `${handle} logged in`,
      time: "2 min ago",
      color: "#06b6d4",
    },
    {
      ...
      icon: "🪙",
      type: "token",
      text: "Earned 150 tokens from bot usage",
      time: "18 min ago",
      color: "#f59e0b",
    },
    {
      ...
      icon: "🤖",
      type: "bot",
      text: "ClawBot processed 12 messages",
      time: "1 hr ago",
      color: "#ef4444",
    },
    {
      ...
      icon: "🚨",
      type: "alert",
      text: "BTC price alert triggered at $98,500",
      time: "2 hr ago",
      color: "#f97316",
    },
    {
      ...
      icon: "💬",
      type: "chat",
      text: "ChatGPT session: 8 exchanges",
      time: "3 hr ago",
      color: "#10b981",
    },
    {
      ...
      icon: "📊",
      type: "portfolio",
      text: "Portfolio updated: +$234.50 today",
      time: "5 hr ago",
      color: "#8b5cf6",
    },
    {
      ...
      icon: "🌍",
      type: "map",
      text: "Profile viewed from Indonesia",
      time: "6 hr ago",
      color: "#3b82f6",
    },
    {
      ...
      icon: "✅",
      type: "task",
      text: "Completed: Set up Telegram bot",
      time: "1 day ago",
      color: "#22c55e",
    },
  ];
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {stats.map((s, i) => (
          <AnimatedStatCard key={s.label} stat={s} delay={i * 80} />
        ))}
      </div>
      <div
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3">
        <div
          className="w-10 h-10 rounded-xl flex items-center justify-center"
          style={{
            background: "oklch(0.12 0.04 180)",
            boxShadow: "0 0 16px rgba(20,184,166,0.3)",
          }}
        >
          <CheckSquare2 className="w-5 h-5 text-teal-400" />
        </div>
        <div>
      ...
      <div
        className="flex gap-2 p-3 rounded-xl border border-teal-500/20"
        style={{ background: "oklch(0.09 0.015 180)" }}
      >
        <input
          type="text"
        ...
        <button
          type="button"
          onClick={addTask}
          className="px-3 py-1.5 rounded-lg text-xs font-medium text-teal-900 transition-all"
          style={{
          ...
          {pending.map((task, i) => (
            ...
            >
              <button
                ...
                className="w-5 h-5 rounded border-2 border-teal-500/50 hover:border-teal-400 transition-colors flex items-center justify-center flex-shrink-0"
  ...
  const [statuses, setStatuses] = useState([
    {
      ...
      status: "checking" as "online" | "offline" | "slow" | "checking",
      latency: 0,
      color: "#ef4444",
    },
    {
    ...
    },
    {
      name: "ICP Network",
      url: "https://ic0.app",
      status: "online" as "online" | "offline" | "slow" | "checking",
      latency: 12,
      color: "#06b6d4",
    },
  ]);
    ...
    const results = await Promise.all(
      statuses.map(async (s) => {
        ...
        try {
          ...
          return {
            ...s,
            status: lat > 2000 ? "slow" : ("online" as "online" | "slow"),
            latency: lat,
          };
        } catch {
          return { ...s, status: "offline" as const, latency: 0 };
        }
      }),
    );
    setStatuses(results);
    ...
    <div className="p-6 space-y-5">
      ...
      >
        <div
          ...
          style={{
            background: allOnline ? "#22c55e" : "#f59e0b",
            boxShadow: `0 0 8px ${allOnline ? "#22c55e" : "#f59e0b"}`,
            animation: "pulse 2s infinite",
          }}
        />
        <span
          className="text-sm font-medium"
          style={{ color: allOnline ? "#4ade80" : "#fbbf24" }}
        >
          {allOnline
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: `0 0 6px ${dotColor(s.status)}`,
                animation:
                  s.status === "checking"
                    ? "pulse 1s infinite"
                    : s.status === "online"
              ...
              {s.latency > 0 && (
                <div className="flex items-center gap-1 mt-0.5">
                  <div
                    ...
                    style={{
                      width: `${Math.min(s.latency / 30, 100)}%`,
                      maxWidth: "120px",
                      background:
                        s.latency < 500
                          ? "#22c55e"
